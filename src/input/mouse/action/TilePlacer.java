		...
		for(int x = gridX-1; x < gridX + 2; x++) {
			for(int y = gridY-1; y < gridY +2; y++) {
				if(gameMap.gridWithinBounds(x, y)) {
					...
					// Mapping table, determins which tile goes where based on a tile sheet of 20
					// A less "elegant" solution to determining which tile should go where. There 
					// is something called a bit shift that is normally used, but it requires that
					// the art be in a certain structure
